...
class PatchRequest<T : Any>(client: OkHttpClient, url: String) : Request<T>(client, url), HasBody {
    ...
    override fun generateRequest(): okhttp3.Request {
        ...
        return generateRequestBuilder().tag(tag).patch(requestBody).url(url()).build()
    }
